...
class CartaoControllerTest {
  ...
  void deveBuscarCartaoPorIdComPanMascaradoEProduto() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/cartoes/{id}", cartao.getId()).with(jwt()))
  ...
  void deveRetornar404QuandoCartaoNaoEncontrado() throws Exception {
    ...
    when(buscarCartaoUseCase.executar(id))
        .thenThrow(new RecursoNaoEncontradoException("Cartão não encontrado"));

    mockMvc.perform(get("/api/v1/cartoes/{id}", id).with(jwt())).andExpect(status().isNotFound());
  }
  ...
  void deveRetornar503ComRetryAfterQuandoProdutoIndisponivel() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/cartoes/{id}", id).with(jwt()))
  ...
  void deveListarCartoesDoPortadorPaginado() throws Exception {
    ...
    when(listarCartoesPorPortadorUseCase.executar(eq(portadorId), any(Pageable.class)))
        .thenReturn(
            new PageImpl<>(
                List.of(new CartaoComProduto(cartao, Optional.of(produto))), pageable, 1));

    mockMvc
        .perform(get("/api/v1/cartoes").param("portadorId", portadorId.toString()).with(jwt()))
        .andExpect(status().isOk())
  ...
  void deveAlterarStatusERetornarCartaoAtualizado() throws Exception {
    ...
    when(alterarStatusCartaoUseCase.executar(cartao.getId(), StatusCartao.BLOQUEADO))
        .thenReturn(cartao);
  ...
  void deveRetornar422QuandoTransicaoDeStatusInvalida() throws Exception {
    UUID id = UUID.randomUUID();
    when(alterarStatusCartaoUseCase.executar(id, StatusCartao.ATIVO))
  ...
  void deveRetornar409QuandoEscritaConcorrenteCausaOptimisticLock() throws Exception {
    UUID id = UUID.randomUUID();
    when(alterarStatusCartaoUseCase.executar(id, StatusCartao.CANCELADO))
