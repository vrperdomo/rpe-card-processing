...
| Frontend | React 19, Vite, TypeScript strict, Tailwind, TanStack Query, React Hook Form + Zod, Vitest, Playwright |

Antes de adicionar **qualquer** dependência: justificar, verificar compatibilidade com Boot 3.5 e fixar a versão no POM agregador (`dependencyManagement`) ou `package.json`.
...
- Workflows: `ci-backend`, `ci-frontend`, `codeql`, `security`, `e2e`, `pr-lint`, `docker-publish`, `release`.
...
- ADRs em `docs/adr/NNN-titulo.md` (Contexto · Decisão · Alternativas · Consequências). Planejados:
